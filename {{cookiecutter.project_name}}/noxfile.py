...
def precommit(session: Session) -> None:
    ...
    args = session.posargs or [
        "run",
        "--config=dev_configurations/.pre-commit-config.yaml",
        "--all-files",
        "--hook-stage=manual",
...
def pyright(session: Session) -> None:
    ...
    session.install(".")
    session.run("pip", "install", "nox", "nox-poetry", "pytest")
    session.chdir(dir="dev_configurations")
    session.run("pyright")
...
def docs_build(session: Session) -> None:
    ...
    session.chdir(dir="docs")

    args = session.posargs or ["-b", "html", "-a", "-j", "auto", "source", "build"]
...
def docs(session: Session) -> None:
    ...
    args = session.posargs or ["--open-browser", "source", "build"]
